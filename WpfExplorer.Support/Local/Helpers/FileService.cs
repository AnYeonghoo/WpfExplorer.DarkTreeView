    ...
    public class FileService
    {
        ...
        public List<FolderInfo> GenerateRootNodes()
        {
            List<FolderInfo> roots = new()
            {
                ...
                CreateFolderInfo(1, "Documents", IconType.TextBox, _directoryManager.DocumentDirectory),
                CreateFolderInfo(1, "Pictures", IconType.Image, _directoryManager.PictureDirectory)
            };
        ...
        public void RefreshSubdirectories(FolderInfo parent)
        {
            ...
            var oldChildrentDict = parent.Children.ToDictionary(c => c.FullPath);
            var newChildrenDict = newChildren.ToDictionary(c => c.FullPath);

            var added = newChildren.Where(c => !oldChildrentDict.ContainsKey(c.FullPath)).ToList();
